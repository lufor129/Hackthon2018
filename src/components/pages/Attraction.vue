<template>
	<div>
      ...
      <section class="jumbotron text-center">
        <div class="container">
          <h1 class="jumbotron-heading">台灣好玩</h1>
          <br>
              ...
              <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    {{catchHotel.Toldescribe}}
                  </div>
                  <div class="modal-body">
                  <p>連絡電話 : {{catchHotel.Tel}}</p>
                  <p>開放時間 : {{catchHotel.Opentime}}</p>
                  </div>
                  <div id="hotelmap">
...
export default{
  ...
  methods:{
    ...
    toggleHotel:function(hotel){
      const vm=this;
      let num=vm.attractions.findIndex((item)=>{
        return item.Name==hotel.Name;
      })
      if(num==-1){
        hotel.stared=true;
        hotel.icon="fas fa-suitcase text-primary"
        hotel.mapicon="attraction"
        vm.attractions.push(hotel);
      }else{
        hotel.stared=false;
        vm.attractions.splice(num,1);
      }
      this.$forceUpdate();
    }
  },
  created(){
    const vm=this;
    const api=`./static/scenic_spot_C_f.json`;
    ...
    vm.$http.get(api).then((response)=>{
      ...
      tempsave.forEach((item,i)=>{
				let word=String(item.Add).substr(0,3);
				if(isNaN(word)==false){
          let tempword=item.Add.substr(3,2);
          if(isNaN(tempword)==false){
            word=item.Add.substr(5,3)
          }else{
    ...
    filterCounty:function(){
      ...
      if(vm.hotel[`${vm.chooseCounty}`]!=undefined){
        ...
        datas.forEach((item,i)=>{
          let stared=this.attractions.findIndex((itemA)=>{
...
.jumbotron {
  ...
  margin-bottom: 0;
  background: url(https://www.taroko.gov.tw/Utility/DisplayImage?id=11359&prefix=original_)  center;
