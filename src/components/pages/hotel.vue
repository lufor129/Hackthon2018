<template>
	<div>
      ...
      <section class="jumbotron text-center">
        <div class="container">
          <h1 class="jumbotron-heading">台灣好住</h1>
          <br>
...
export default{
  ...
  methods:{
    ...
    toggleHotel:function(hotel){
      const vm=this;
      let num=vm.hotels.findIndex((item)=>{
        return item.Name==hotel.Name;
      })
      if(num==-1){
        hotel.stared=true;
        hotel.icon="fas fa-building text-danger"
        hotel.mapicon="hotel";
        vm.hotels.push(hotel);
      }else{
        hotel.stared=false;
        vm.hotels.splice(num,1);
      }
      this.$forceUpdate();
    }
  },
  created(){
    const vm=this;
    const api=`./static/hotel_C_f.json`;
    ...
    filterCounty:function(){
      ...
      if(vm.hotel[`${vm.chooseCounty}`]!=undefined){
        ...
        datas.forEach((item,i)=>{
          let stared=this.hotels.findIndex((itemA)=>{
...
.jumbotron {
  ...
  margin-bottom: 0;
  background: url(http://jerome.anyday.com.tw/gallery/albums/20041202-1/20041202_a1.jpg)  center;
